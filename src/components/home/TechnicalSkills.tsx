...
export const TechnicalSkills: React.FC<TechnicalSkillsProps> = ({ technical, id }) => {
  if (!technical.display) return null;

  return (
    <section className="flex flex-col w-full mb-16" aria-labelledby={id || "tech-skills-title"}>
      <h2
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-4" aria-label="List of technical skills">
        {technical.skills.map((skill) => (
          ...
          >
            {skill.icon && iconLibrary[skill.icon as keyof typeof iconLibrary] && (
              <div className="text-[var(--brand-medium)]" aria-hidden="true">
                {React.createElement(iconLibrary[skill.icon as keyof typeof iconLibrary], {
                  size: 18,
                })}
              </div>
            )}
